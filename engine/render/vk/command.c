...
--*/
{
    ...
    CommandPoolCreateInformation.sType =
        VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
    CommandPoolCreateInformation.flags =
        VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
    CommandPoolCreateInformation.queueFamilyIndex =
        VlkData.Gpu->GraphicsFamilyIndex;

    LogTrace("Creating primary command pool");
    VULKAN_CHECK(
        vkCreateCommandPool(VlkData.Device, &CommandPoolCreateInformation,
                            VlkGetAllocationCallbacks(), &VlkData.CommandPool));
    VlkSetObjectName(VlkData.CommandPool, VK_OBJECT_TYPE_COMMAND_POOL,
                     "Command pool");
    ...
    VULKAN_CHECK(vkCreateCommandPool(
        VlkData.Device, &CommandPoolCreateInformation,
        VlkGetAllocationCallbacks(), &VlkData.TransferCommandPool));
    VlkSetObjectName(VlkData.TransferCommandPool, VK_OBJECT_TYPE_COMMAND_POOL,
...
--*/
{
    ...
    VULKAN_CHECK(vkAllocateCommandBuffers(VlkData.Device,
                                          &CommandBufferAllocateInformation,
                                          VlkData.CommandBuffers));
    for (i = 0; i < VULKAN_FRAME_COUNT; i++)
    {
        VlkSetObjectName(VlkData.CommandBuffers[i],
                         VK_OBJECT_TYPE_COMMAND_BUFFER, "Command buffer %u", i);
    }
    ...
    for (i = 0; i < VULKAN_FRAME_COUNT; i++)
    {
        VULKAN_CHECK(vkCreateFence(VlkData.Device, &FenceCreateInformation,
                                   VlkGetAllocationCallbacks(),
                                   &VlkData.CommandBufferFences[i]));
        VlkSetObjectName(VlkData.CommandBufferFences[i], VK_OBJECT_TYPE_FENCE,
                         "Command buffer fence %u", i);
    }
}