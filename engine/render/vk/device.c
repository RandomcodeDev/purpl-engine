...
--*/
{
    ...
    for ( i = 0; i < stbds_arrlenu(Devices); i++ )
    {
        ...
        if ( CurrentGpu->Usable )
        {
            ...
            VlkData.GpuIndex = i;
            LogDebug("Selected device %zu %s [%04x:%04x]", VlkData.GpuIndex, VlkData.Gpu->Properties.deviceName, VlkData.Gpu->Properties.vendorID, VlkData.Gpu->Properties.deviceID);
            break;
        }
    }
